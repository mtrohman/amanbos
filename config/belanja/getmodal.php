...
$ta = $_SESSION['ta'];
// $ta = isset($_GET['ta']) ? intval($_GET['ta']) : 2019;
...
// 'npsn', 'like', '%' .$npsn. '%'
$belanja = Belanja::thBerjalan()->with(['rka.program','rka.kp','rka.sekolah','rka.rekening'])->ta($ta)->triwulan($triwulan)->npsn($npsn)->namaSekolah($sekolah)->modal()->skip($offset)->take($rows)->get();

$belanjacount = Belanja::thBerjalan()->with(['rka.program','rka.kp','rka.sekolah'])->ta($ta)->triwulan($triwulan)->npsn($npsn)->namaSekolah($sekolah)->modal()->count();
