...
<body class="fix-header card-no-border">
                            ...
                            <div class="card">
                                <div class="d-flex flex-row">
                                    ...
                                    <div class="align-self-center m-l-20">
                                        <h5 class="m-b-0 text-info"><?=rupiah($sisapagu3);?></h5>
                            ...
                            <div class="card">
                                <div class="d-flex flex-row">
                                    <div class="p-10 bg-warning">
                                        <h3 class="text-white box m-b-0"><i class="ti-wallet"></i></h3></div>
                                    <div class="align-self-center m-l-20">
                                        <h5 class="m-b-0 text-warning"><?=rupiah($sisapagu4);?></h5>
                                ...
                                <div class="row">
                                    <div class="col-lg-12">
                                        <div class="text-center text-md-left db">
                                            ...
                                            <h1 class="h1special pull-right d-none d-md-block">RKA Perubahan</h1>
                                        </div> 
                                    </div>
                                </div>
                                <hr>
    ...
    <script>
        ...
        $('#dg').datagrid({
            url:'config/rkaperubahan/getdata.php?npsn='+npsn ,
		...
		function newRka(){
            ...
            $('#forminput').attr('action','config/rkaperubahan/save.php');
            row = 1;
            tambahBaris();
        ...
        function saveRka() {
            ...
            if (!empty && !twempty) {
                if ($('#tw').val()=='3' || $('#tw').val()=="4") {
                    $("#template").detach();
                    $( "#forminput" ).submit();
                }
                else{
                    $("#spanvalidasi").text("Mohon mengisi triwulan 3 atau 4").show().fadeOut( 3000 );
                }  
            }
            else{
        ...
        function editRka() {
            ...
            if (data){
                ...
                $('#forminput').attr('action','config/rkaperubahan/update.php?id='+data.id);
                row = 1;
                tambahBaris();
        ...
        function destroyRka(){
            ...
            if (row){
                $.messager.confirm('Confirm','Apakah anda akan menghapus data rka '+row.sekolah.nama_sekolah+' dengan rekening: '+row.nama_rekening+'?',function(r){
                    if (r){
                        $.post('config/rkaperubahan/destroy.php',{id:row.id},function(result){
                            if (result.success){
