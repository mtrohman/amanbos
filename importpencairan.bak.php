...
include_once 'ceklogin.php';
    ...
    if (!empty($_POST)) {
        ...
        if($datalama['jumlah']>0){
            ...
            if ($hapus) {
                # code...
                $hapus1= mysqli_query($link,"DELETE FROM pagutriwulan WHERE ta='$tahun' AND triwulan='$triwulan'");
                if($hapus1){
        ...
        if ($FileType=="xlsx") {
            ...
            $sql="";
            $sqlabc="";
            $sql.="INSERT INTO pencairanrka
            (ta, triwulan, npsn, belanja_pegawai, belanja_barangjasa, belanja_modal) VALUES ";
            $sqlabc.="INSERT INTO pagutriwulan
            (ta, triwulan, npsn, belanja_pegawai, belanja_barangjasa, belanja_modal) VALUES ";
            
            for ($row = 2; $row <= $highestRow; ++$row) {
                // echo '<tr>' . PHP_EOL;
                $sql.="('$tahun','$triwulan',";
                $sqlabc.="('$tahun','$triwulan',";
            ...
            if($result){
                $result1= mysqli_query($link,$sqlabc);
                if ($result1) {
                    // echo "<br>Sukses";
                    $sukses=1;
